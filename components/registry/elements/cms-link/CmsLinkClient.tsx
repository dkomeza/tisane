...
export function CmsLinkClient({ data }: BlockProps<CmsLinkProps>) {
  ...
  return (
    <Link
      ...
      rel={rel}
      className={cn(
        "flex flex-col gap-1 group hover:text-brand-purple-300 @md:hover:text-brand-purple-200 transition-colors",
    ...
    >
      ...
      <span
        className={cn(
          "w-full @md:w-0 h-0.5 group-hover:w-full transition-all",
          isActive
            ? "@md:w-full bg-brand-purple-300 @md:bg-brand-purple-200"
            : "bg-brand-grey-100 group-hover:bg-brand-purple-300 @md:group-hover:bg-brand-purple-200",
        )}
      ></span>
    </Link>
  );
