/**
 * Column Admin Component — renders children in actual flex-col layout
...
export default function ColumnAdmin({
  ...
  return (
    <div className="relative group/column w-full">
      {/* Settings trigger — top-right corner */}
      <div className="absolute -top-4 right-1 z-10 flex items-center gap-1 opacity-0 group-hover/column:opacity-100 transition-opacity duration-200">
        <Popover>
      ...
      {/* Hover outline */}
      <div className="rounded-md ring-2 ring-transparent group-hover/column:ring-primary/30 transition-all duration-200 p-1">
        {/* Actual flex-col layout matching the client */}
        <div
          className={cn(
        ...
        >
          ...
          {block.data.children?.map((child, index) => {
            ...
            return (
              <div key={childId} className="flex flex-col gap-0 w-full">
                <InsertionLine
                  onInsert={(newBlock) =>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button
                type="button"
                className="w-full h-10 shrink-0 border-2 border-dashed border-border/40 rounded-lg flex items-center justify-center hover:border-primary/50 hover:bg-primary/5 transition-all"
              >
